...
verify precisely because it is expensive to fake.
...
placement law), and order vectors (permuted arrival, identical
Constitutions). A guard that has never been shown failing is
not yet a guard; the vectors are the exhibit that every wall in
...
into it. Mechanical census at meld close: of the predecessor's
1,546 nonblank lines, 177 lack a byte-identical counterpart in
this candidate; each maps to exactly one accounted group below;
zero unexplained. Every other predecessor sentence was re-derived
...
7. Meld recompositions (125 lines, this integration station) —
   ruled: the manifest's Amendment 1 and the meld instruction;
   each subgroup accounted:
