...
Two notes bind at this heading, outside the seed's bytes.
First, vocabulary: this chapter's "substrate" and "medium"
usages are layer-role uses bound forward to the
substrate-of-record definition in section 4 — the binding
block sits after this chapter in reading order, and a linear
reader meets the vocabulary here first. Second, routing: the
aggregate-Constitution construction — the clause-set head as
an aggregate commitment over per-clause u-salted,
SAID-addressed sub-blocks, repairing this chapter's
clause-selective and admitted disclosure postures — is ruled
(ruling record supplement 3, S3-3) and enters this chapter
only through the seed's own regeneration station, never by
edit of the graduated bytes below; the kernel-side halves of
the ruling land in sections 7 and 13.
...
presentation forms the consumption ladder of this standard types —
...
2. Digest pins name exact bytes, and this document uses two pin
   ...
   the number sign, CESR's own dummy convention — under
   CESR's length-parametric rule by derivation code, of which
   forty-four characters, the qb64 Blake3-256 digest class of
   CESR's code table, is
...
is cited by name and never restated: substrate components are
cited by their proper names — KERI, ACDC, CESR, and the named
mechanism — at every load-bearing use, and the role-abstract
vocabulary ("the substrate," "the medium") remains lawful only
where the claim genuinely holds for any conformant substrate
of this class.
...
- **GEL** (extension) — governance event log. The committed
  ...
  same anchoring discipline KERI's registry layer uses
...
separation is KERI's own controller-validator division
...
that grounds cite and disclosure binds to, and the sub-block
the aggregate-Constitution commitment of section 7 ranges
over. A covenant is the
...
named beside KERI's own seal grammar, the first two
subsuming kinds KERI ships, the third this standard's
...
**Duplicity** (KERI term, extended by tier). Two voices
...
receipt-consistent. Its clauses bind the witness state each
stratum's establishment events already commit — a per-stratum
floor over KERI's witness set and threshold fields — and its
...
identically from the same committed inputs under KERI's
pinned semantics, because their agreement is cryptographic rather
than negotiated. One distinction is stated before the
properties: which events an observer has accepted is
observer-relative — KERI's acceptance state is local,
first-seen and best-available by design — while the fold over
an accepted set is deterministic: given the same complete
accepted event set, every verifier's fold agrees. A KEL folds
to the same key state everywhere; a
...
KERI's own: its superseding-recovery rules — KERI's
superseding recovery, specified in the KERI specification's
superseding-recovery section and named here at first use;
every later cite of the recovery calculus in this document
means this named rule set — lawfully
...
by an identifier's key state is decided by KERI's own
machinery, identically for every observer holding the same
accepted events — so the first half of
...
- **Verification cone** (of an edict) — the minimal committed log
  ...
  in place of it. A cone root SHALL carry a typed
  authentication grade of four components — controller
  threshold verified; witness roster identified from the
  applicable establishment state; receipt threshold satisfied;
  establishment coordinate cited — and where the receipt
  threshold is unmet, the grade discharges as a pending
  finding whose typed requirement enumerates the missing
  receipt indices. A fresh verifier needs
...
- **Warranty** — a signed attestation of a computed finding,
  ...
  the judgment. Replay-falsifiable by construction. Its KERI
...
attestation in ACDC's credential discipline — typed by
schema identifier, revocable through its registry, its lens cited
by edge. A seat likewise takes credential form where standing
must be toolchain-checkable: a seat credential names the
organ's AID as issuee, issued under the domain's registry, and
a warranty's edge to its warrantor's seat credential SHALL
carry the ACDC edge operator DI2I — the citing credential's
issuer must be the pointed-to credential's issuee or a
delegated AID thereof — so that "the warrantor holds the seat
it claims" is checked by edge validation in the existing
...
unmerged. Requirement elements in typed requirement sets SHALL
name their required schemas by schema identifier in the same
discipline. Object forms typed this way are consumable by
KERI's existing toolchain; nothing here requires a bespoke
parser.
...
provides. Admission enactments MAY grant disclosure under
chain-link confidentiality — ACDC's contractually protected
disclosure, in which the disclosed object's rules carry the
admission terms — so that an admitted party's unpermitted
onward disclosure is breach with committed evidence; where the
admitted posture of Chapter 2 narrows its jury, this is the
discipline that makes leaks from the narrowed jury
convictable. A cone span withheld under such a profile is
...
SHALL carry a blinding factor in ACDC's own discipline for
exactly this — the high-entropy UUID field, the u-field, that
defeats dictionary recovery of a committed identifier's
preimage — cited, not
...
**The aggregate Constitution.** Where the committed
Constitution's clause-set head is the commitment
clause-selective disclosure opens against, that head SHALL be
an aggregate commitment over per-clause sub-blocks (the
aggregate's digest function and concatenation order are
semantics this document owes and the encoding round pins —
an openness of commitment form, confessed here, not a carriage
detail): each
clause independently SAID-addressed (the clause definition of
section 5 already commits this) and carrying a per-clause
blinding factor under the u-field mandate above — the two
ratified clauses compose. Clause-selective disclosure then
reveals the ground-cited clauses with their blinding factors;
the verifier recomputes membership in the anchored aggregate;
undisclosed clauses remain provably present and unreadable. A
monolithic digest admits no partial verification, and
revealing a clause against one proves only that the clause was
committed somewhere, never its membership in the law in force.
The construction's carriage is chartered to the encoding
round; the disclosure postures it serves live in Chapter 2 and
receive it through that chapter's own station.
...
- **self-convicted(proof)** — the subject's own committed bytes
  ...
  is decided by KERI's own superseding-recovery rules —
...
governance events — instantiated, never returned: KERI's and
the registry layer's acceptance machinery do not return this
document's finding type, and their operational outcomes
(acceptance, escrow, validation failure, missing receipts)
enter appraisal as typed evidence facts; the fold computes
findings over those facts. The evidence ordering below is stated once and
holds per tier.
...
of the missing evidence; **window-open** is cured when
KERI's superseding rules no longer admit a superseding event
...
when no lawful superseding rotation remains admissible under
KERI's delegated-recovery rules, which stay open longer (KERI's
own superseding-recovery calculus is the decision procedure;
...
convicts under the tier's committed rules (at the key tier,
KERI's superseding-recovery calculus; at the registry and
...
conviction. Extending
KERI's word up the tiers does not extend KERI's
force.
...
committed way of expressing it. The threshold constructions at the two
ends of the system are analogous — the same satisfaction shape
over differently typed slot judgments, never one algebra: a
KERI signing slot asks whether a key signed; an edge slot here
asks whether a credential stands, and that is this document's
own fold question — schema, issuer qualification, registry
state, disclosure state — never the substrate's. When the
shared satisfaction predicate exists as its own committed
artifact upstream, both sites cite it by digest and the
analogy upgrades to a declared shared dependency by ordinary
migration, no re-ruling. An unsatisfied operator group is not a defect and not
...
kinds verifies nothing. KERI's seal grammar ships more
...
cited, never renamed. One KERI kind is expressly out of
...
**Digest seal** (KERI vocabulary). A commitment to exact
...
**Event seal** (KERI vocabulary). A commitment to an event
...
typed seal structure KERI itself ships — the generic typed
seal, a type field and a digest field, traveling under its own
count code in CESR's count-code table — with a reserved type
value, its seal data the self-addressing identifier of the
sealed clause set — no KERI extension is claimed, and a
...
KERI admission below, frame appraisal above, one set of
bytes — pointed at the frame's own keys: KERI admits the
...
**The degenerate species.** Bare pre-rotation — KERI's
...
compromised organ inside KERI's own delegated-recovery
rules, and gives the charter's delegation strata KERI's
delegation semantics rather than a metaphor. The seat grant
itself takes credential form where the objects section types
it: a seat credential naming the organ's AID as issuee, issued
under the domain's registry — the object the DI2I edge of
section 7 points at. Where an organ is a
delegated identifier, the who-may-invoke and what-defeats-it axes
are partially discharged by KERI itself — and the
degenerate policy's "defeasible by nothing" acquires its
exception, since KERI already imposes a richer policy on
delegated identifiers than on sovereign ones.

**The delegated gAID** (an OPTIONAL profile). A born-governed
domain whose gAID is itself a delegated identifier of a
custodial quorum MAY adopt that shape as its recovery
constitution: delegated establishment requires the delegator's
anchor, so the domain's constitutional recovery windows are
enforced by KERI's delegation physics rather than by committed
policy appraised after the fact. The cost is confessed in the
same breath: the delegator becomes infrastructure the domain's
autonomy must account for; the sole-authority clause pattern
extends to the custodial quorum; and the two-identifier shadow
analysis re-runs at the delegator. Some domains will rightly
refuse the dependency; the profile exists so the choice is a
committed act rather than an accident of inception shape.
...
two-layer joint of section 11 pointed at a different KERI
object: KERI admits X-events by its own mechanics; the
...
KERI mechanics and never requires KERI's permission
...
exactly when three things hold: X has a KERI lifecycle —
committed events with KERI's own admission mechanics; X's
...
- **Governed witness configuration** — the witness set and
  threshold fields of establishment events, already committed
  key state; its law is the availability charter's per-stratum
  floor (section 15), and an establishment event whose new
  witness state falls below the chartered floor is a
  convictable governance event on the same KEL bytes.
...
KERI's own admission rules: the degenerate law every
frame has because KERI imposes it, in the pattern of
...
commits law over a further KERI lifecycle has derived the
...
1. **Authenticate** (medium, frame-invariant): B MUST verify E's
   ...
   identical in every frame; it is KERI's own admission
   machinery — its outcome enters appraisal as a typed
   admission fact, never as a finding of this document's
   codomain — and its verdict does not depend on B's law.
...
3. **Appraise** (frame-local): B's evaluator SHALL compute its
   ...
   in B's adoption covenant may be expressed in ACDC's
...
Observation is typed, never presumed. The cross-frame
discipline's evidence production is given two committed object
forms, both schema-typed evidence admissible in an evidence
bundle like any other: the watcher discrepancy report — a
watcher's committed, signed record that the key state it
observed for an identifier diverges from another observed
state, carrying both observed states and their source
coordinates — and the key-state-notice comparison package —
the paired key-state notices themselves, committed together as
the comparison's ground, so the divergence is recomputable
from the package rather than believed from the report. Where a
domain seats judges or jurors over disputed reports, those
seats are constructor-plane organs: their determinations are
replayable evidence the fold consumes, never fold
replacements. The acquisition procedure — how a verifier comes
to hold both branches, by what endpoints, under what freshness
discipline — is expressly confessed open, pending maturation
of the observation roles upstream; a confessed open procedure
over typed evidence beats an invented closure.
...
Steps 1-2 bind to KERI's own machinery; steps 3-4 bind to
...
The composed joint — KERI admission and governance appraisal
...
KERI commits detection and its own key-layer recovery,
...
- **In the medium** — cross-frame duplicity convicts its author
  ...
  (modulo KERI's superseding reconciliation, per the
...
been made available — and its clauses bind the witness state
each stratum's establishment events already commit: a
per-stratum floor — a minimum witness threshold and a
set-stability rule over KERI's committed witness fields, the
witness set and threshold an establishment event carries — so
that an establishment event whose new witness state falls
below the chartered floor is a convictable governance event on
the same KEL bytes, derivable by any holder (the governed
witness configuration of section 12). The scope is stated
exactly: witness receipts prove receipt, never serving; the
charter binds what the medium commits, and its discharge is
appraised like any clause, never presumed from the witnesses'
existence. Delegated authority carries the
...
committed deliverable of the deployment lane, and KRAM —
KERI's own request-authentication mechanism, whose timed-cache
discipline is exactly the replay-attack taxonomy and
cache-window discipline this obligation names — is the design
of record for it, cited, not restated. One boundary binds
here: transport admission MAY be wall-clock-windowed, and the
window is KRAM's cache window, cited rather than restated;
appraisal
...
charter's endpoints rides OOBI — KERI's out-of-band
introduction machinery — cited at the same grain.

**Interpretive latitude.** Where KERI's behavior was
...
receipt-race edges, escrow retention) remains open wherever
KERI's own law is silent. A federated GARD SHALL state
...
predicate set consumed — KERI's superseding-recovery
...
posture is CESR's native composable attachment grammar
rather than document envelopes, chartered to a design round
convened with CESR's own stewards and this standard's
...
seven walls has inferred more than this document states, and the
...
- **Track one — registry-form reuse.** GEL events use
  KERI's registry event forms under their existing ilks,
  ...
  One confession binds the election: reuse is semantically
  loaded for governance-blind consumers — the reused ilks carry
  their native registry semantics, and a registry-capable
  consumer may act on them as registry state — so the
  colorless-base claim is scoped to parseability, never to
  meaning-neutrality, and a domain electing this track owns
  that load as committed law.
...
**Genus.** CESR's genus namespace — the externally stewarded
table through which a parser sizes and dispatches every count
code — admits reservation, and a reservation of a governance
genus is itself an enactment: committed in the GEL of the
domain that reserves it, carrying the reserved coordinates and
their intended grammar, judged and superseded under that
domain's law. The enactment binds the reserving domain's own
profile and nothing wider: it is a reservation in a table
someone else stewards, and it cannot by itself create
interoperable CESR meaning. Recognition of the reservation by
CESR's stewards is a distinct, later, bilateral event — a
federation-shaped recognition, not a precondition for local
use, and the interoperability gate for everything beyond it.
The reservation this standard's own record commits is
Exhibit-class evidence for section 16's confessed frontier:
enacted, unrecognized, and honest about the difference. The
covenant seal's reserved type value (section 10) lives in a
namespace of the same kind — the typed seal structure's type
table, externally stewarded — and carries the same recognition
question; it is stated here beside the genus so the two
reservations are answered together, never separately by
accident.
...
envelope form — KERI's own receipt bytes unmodified,
...
closes is verified from keripy, KERI's reference
implementation, at the pinned checkout 79e31cc8: KERI's receipt
form is the one event form whose identifier field is not
self-addressing — a receipt's identifier names the receipted
event, not the receipt's own bytes — and nothing in
KERI today lets one digest address a receipt together with
...
> receipt is the Blake3-256 SAID, qb64, of the canonical bundle
> preimage: the concatenation,
> in this committed order, of the receipt serialization bytes
> exactly as receipted — the KERI receipt form unmodified, wrapped
> and never altered — followed by each attachment group's framed
> bytes, count code included, the groups ordered
> lexicographically over the qb64 of each group's
> primary identifier (for an indexed-signature group,
> the signer's prefix; for a couple, the coupled identifier;
> and for any other CESR attachment-group class, the group's
> primary identifier as fixed by the encoding round's carriage
> profile before that class may appear in a bundle — a bundler
> meeting a group class the profile does not enumerate refuses,
> it never chooses);
> where a group's own grammar commits an
...
construction rather than by discipline; KERI
...
recomputes bundle identifiers against receipts produced by
keripy, KERI's reference implementation, at a pinned checkout —
79e31cc8 is the checkout of record at this writing — and
...
envelope fed to an ungoverned KERI node at the pinned
...
workaround for a KERI gap; it is the only geometry the
...
infeasible. At most one coverage can be interior, and
KERI chose: identity covers all content, with no holes,
...
unrecognized group, and keripy's parser raises
on unexpected count codes rather than skipping them — verified
from keripy's primary bytes at the pinned checkout 79e31cc8. Skippability is
...
invariant. Anchors: governance commitments as seals in
KERI's own events, parsed, replicated, witnessed, and
duplicity-guarded by every ungoverned node natively — the
governance tier's security inherited by anchoring, reimplemented
nowhere. The heavyweight envelope: KERI receipt bytes
...
falls as commitment deepens; meaning never moves; and
KERI's protections hold undiminished at every rung,
...
Zero collisions unexamined at assembly. One carried span was
found stale by the 42-3 gauntlet after this census closed (the
walls count of section 17, repaired in the v3 pass under its
census below) — the totality claim is scoped to the assembly
pass, and later rounds convict later.
...
### Repair census (supplement 3 pass, 2026-08-05)

This section is appended under the same law as the three above:
the assembly censuses stand untouched; this census accounts every
change from candidate v1 (sha256 8b4f701a…) to v2, one row per
substitution, each citing its ruling in supplement 3 (sha256
79c7d7bd…) and its finding in the 42-2 round record. The graduated
seed block is byte-identical across the pass; the ratifying
authority's abstract edit stands.

| id | ruling | finding | site | change |
|---|---|---|---|---|
| L1 | S3-3; S3-8 | A-12; A-9 | Chapter 2 integration heading | Pre-binding vocabulary note + confessed S3-3 routing note at the heading |
| A1 | S3-8 | A-4 | §4 pin rule | Dummy convention named as CESR's; 44-character class named qb64 Blake3-256 |
| A2 | S3-8 | A-1 | §4 substrate of record | The direct-naming rule enters the citation discipline at its license site |
| A3 | S3-8 | A-13 | §5 GEL entry | substrate's registry layer -> KERI's registry layer |
| A4 | S3-8 | A-13 | §5 evaluator/constructor | substrate -> KERI (controller-validator division) |
| A5a | S3-3 | A-9; SOL A3 | §5 clause definition | Clause named as the sub-block the §7 aggregate commitment ranges over |
| A5 | S3-8 | A-13 | §5 seal kinds | substrate -> KERI (seal grammar, twice) |
| A6 | S3-8 | A-13 | §5 duplicity entry | substrate term -> KERI term |
| A7 | S3-5 | B-4; SOL A20 | §5 availability-charter entry | "witnesses discharge it" overclaim scoped: clauses bind committed witness fields |
| A8 | S3-8 | SOL A1 | §6 medium identity | Observer-relative acceptance distinguished from deterministic folding |
| A9 | S3-8 | A-5 | §6 superseding recovery, first use | Superseding recovery named with its spec section at first use |
| A10 | S3-8 | A-13 | §6 medium/achievement | substrate -> KERI (the medium is KERI's achievement) |
| A11 | S3-8 | SOL A1 | §6 frame-invariant authentication | substrate -> KERI; "identically" scoped to the same accepted set |
| B1 | S3-8 | A-13 | §7 warranty lineage | substrate lineage -> KERI lineage |
| B2 | S3-5 | SOL B1; SOL A19 | §7 verification cone root | Cone-root typed authentication grade (four components; missing-receipt indices pending) |
| B3 | S3-4; S3-8 | B-3; A-7 | §7 object typing (warranty) | Credential discipline named ACDC; seat credential form; DI2I on warranty->seat edge; evidence-not-verdict |
| B4 | S3-8 | A-13 | §7 object typing (toolchain) | substrate's existing toolchain -> KERI's existing toolchain |
| B5 | S3-3 | B-1; SOL B4 | §7 disclosure posture | Chain-link confidentiality for admitted-posture grants (kernel-side S3-3 half) |
| B6 | S3-8 | A-7; SOL A9 | §7 blinding mandate | Blinding factor named as ACDC's u-field (the field, not "salt-field discipline") |
| B7 | S3-3 | B-1; SOL A3; SOL A4 | §7 end (aggregate Constitution) | Aggregate-Constitution construction at the commitment-form site; composing clauses cited |
| C1 | S3-8 | A-5 | §8.1 self-convicted ground | substrate -> KERI (superseding-recovery rules) |
| C2 | S3-8 | SOL A10 | §8.1 tier instantiation | Adapter sentence: acceptance machinery does not return the finding type; typed evidence facts mediate |
| C3 | S3-8 | A-5 | §8.2 window-open cure (1) | substrate -> KERI (superseding rules) |
| C4 | S3-8 | A-5 | §8.2 window-open cure (2) | substrate -> KERI (delegated-recovery rules; recovery calculus) |
| C5 | S3-8 | A-5 | §8.3 bearing rule | substrate -> KERI (superseding-recovery calculus) |
| C6 | S3-8 | A-13 | §8.4 force distinction | substrate -> KERI (word/force, twice) |
| D1 | S3-1 | SOL A12; A-10 | §9 one-algebra site | One-algebra claim scoped to analogous constructions; slot-judgment sentence; forward mint clause |
| D2 | S3-8 | A-13 | §10 seal grammar | substrate -> KERI (seal grammar) |
| D3 | S3-8 | A-13 | §10 out-of-scope kind | substrate kind -> KERI kind |
| D4 | S3-8 | A-13 | §10 digest seal | substrate vocabulary -> KERI vocabulary |
| D5 | S3-8 | A-13 | §10 event seal | substrate vocabulary -> KERI vocabulary |
| D6 | S3-8 | A-8 | §10 covenant seal carriage | Generic typed seal named as KERI's (t, d) structure with its CESR count-code citation |
| E0 | S3-8 | A-13 | §11 two-layer joint | substrate admission -> KERI admission (twice) |
| E1 | S3-8 | A-13 | §11 degenerate species (1) | substrate -> KERI (next-key digest) |
| E2 | S3-8 | A-13 | §11 degenerate species (2) | substrate -> KERI (imposes the degenerate policy) |
| E3 | S3-4; S3-8 | B-3; SOL B7; A-13 | §11 seat site | substrate -> KERI (twice); seat credential form stated at the seat site |
| E4 | S3-8 | A-13 | §11 axes discharged | substrate -> KERI (twice) |
| E5 | S3-6 | B-5; SOL B8 | §11 delegated-gAID profile | The delegated-gAID MAY-grade profile with its confessed cost |
| F1 | S3-8 | A-13; SOL A16 | §12 the one move | substrate -> KERI (twice) |
| F2 | S3-8 | A-13; SOL A16 | §12 governed X | substrate -> KERI (twice) |
| F3 | S3-8 | A-13; SOL A16 | §12 criterion | substrate lifecycle -> KERI lifecycle; admission mechanics named |
| F4 | S3-5 | B-4; SOL A17 | §12 class enumeration | Witness configuration admitted as a governed object class under the criterion |
| F5 | S3-8 | A-13 | §12 reflexive base case | substrate -> KERI (twice) |
| F6 | S3-8 | A-13 | §12 open enumeration | substrate lifecycle -> KERI lifecycle |
| G1 | S3-8 | A-13; SOL A19 | §13.2 step 1 (authenticate) | substrate -> KERI; admission outcome typed as evidence fact, not a finding |
| G2 | S3-8 | A-13 | §13.2 step 3 (appraise) | substrate's edge-operator grammar -> ACDC's edge-operator grammar |
| G3 | S3-8 | A-13 | §13.5 engine independence (1) | substrate -> KERI (steps 1-2 machinery) |
| G4 | S3-8 | A-13 | §13.5 engine independence (2) | substrate admission -> KERI admission |
| G5 | S3-7 | SOL B2; A-2 kin | §13.4 cross-frame conviction site | Watcher discrepancy report + KSN comparison package typed; judges/jurors constructor-plane; acquisition confessed open |
| H1 | S3-8 | A-5 | §14.2 medium rung | substrate's superseding reconciliation -> KERI's |
| H2 | S3-8 | A-13 kin; A-5 | §14 opening (detection and key-layer recovery) | The substrate commits -> KERI commits (load-bearing: KERI-specific detection + recovery) |
| I1 | S3-5 | B-4; SOL A20 | §15 availability charter (discharge overclaim) | "witnesses discharge it" scoped: clauses bind committed wits/toad floor; floor violation convictable from KEL bytes |
| I2 | S3-8 | A-6; SOL A19 | §15 request authentication (KRAM) | Request-authentication mechanism named KRAM; wall-clock window cites KRAM cache-window semantics |
| I3 | S3-8 | A-6; SOL A21 | §15 endpoint discovery (OOBI) | Out-of-band introduction machinery named OOBI |
| I4 | S3-8 | A-13 | §15 interpretive latitude (1) | substrate's behavior -> KERI's behavior |
| I5 | S3-8 | A-13 | §15 interpretive latitude (2) | substrate's own law -> KERI's own law |
| I6 | S3-8 | A-5 | §15 functional-dependency declaration | substrate's superseding-recovery calculus -> KERI's |
| J1 | S3-8 | A-13; SOL A23 | §16 what is open (attachment grammar) | substrate's native composable attachment grammar -> CESR's |
| J2 | S3-8 | A-13 | §16 what is open (stewards) | substrate's own stewards -> CESR's own stewards |
| K1 | S3-8 | SOL A24 | §18 track one (confession) | Track-one reuse confessed semantically loaded; colorless-base claim scoped to parseability |
| K2 | S3-8 | A-2; SOL A25; A-8 | §18 genus paragraph | Genus named CESR; reservation scoped to externally stewarded table; recognition bilateral; seal-type namespace stated beside it |
| K3 | S3-8 | A-13; SOL A24 | §18 track one (registry forms) | substrate's registry event forms -> KERI's registry event forms |
| N1 | S3-8 | A-13 | §19 envelope form (head) | substrate's own receipt bytes -> KERI's own receipt bytes |
| N2 | S3-8 | A-3 | §19 gate one (reference implementation) | Reference implementation named keripy at 79e31cc8; substrate -> KERI (twice) |
| N3 | S3-8 | A-3 | §19 bundle rule (Blake3-256, qb64, per-group enumeration) | Blake3-256 + qb64 restored to the bundle-preimage rule; per-group primary-identifier enumeration restored verbatim from the gates doctrine |
| N4 | S3-8 | A-13 | §19 vector obligation (non-interference) | substrate non-interference -> KERI non-interference |
| N5 | S3-8 | A-3 | §19 gate one stands (reference implementation) | Reference implementation named keripy; pinned checkout 79e31cc8 named |
| N6 | S3-8 | A-13 | §19 vanilla passthrough | ungoverned substrate node -> ungoverned KERI node |
| N7 | S3-8 | A-13 | §19 one-way-coverage ground (1) | substrate gap -> KERI gap |
| N8 | S3-8 | A-13 | §19 one-way-coverage ground (2) | the substrate chose -> KERI chose |
| N9 | S3-8 | A-3 | §19 transport stratum (raise-not-skip) | Reference parser named keripy; pinned checkout named at the raise-not-skip fact |
| N10 | S3-8 | A-13 | §19 anchors voice | substrate's own events -> KERI's own events |
| N11 | S3-8 | A-13 | §19 heavyweight envelope voice | substrate receipt bytes -> KERI receipt bytes |
| N12 | S3-8 | A-13 | §19 closing protections | substrate's protections -> KERI's protections |

Entries: 74 — 44 naming
conversions, 22 prose repairs,
8 insertions. S3-9's ripened items
are deliberately absent: they await their owed ruling.

### v3 micro-pass census (42-3 gauntlet repairs, 2026-08-06)

Appended under the same law as the four censuses above. Every
change from v2 (sha256 a11f2902...) to v3, one row per
substitution, each citing its 42-3 gauntlet finding
(reviews/rounds/42-3-full-gauntlet-2026-08-05/gauntlet-report.md,
sha256 f5220ca6...). The seed chapter's repairs travel separately
through its station (round 42-4); GF-7 and GF-8 are ruled
residuals bounded by the encoding round and the station ledger
respectively; GF-1 routes to the seed station.

| id | finding | site |
|---|---|---|
| V3-1 | GF-5 | §8.2 window-open cure (1) |
| V3-2 | GF-5 | §8.2 window-open cure (2) |
| V3-3 | GF-5 | §8.4 force distinction |
| V3-4 | GF-5 | §19 closing protections |
| V3-5 | GF-2 | §17 walls count |
| V3-6 | GF-2 | appendix collision census closing |
| V3-7 | GF-3 | §19 bundle-preimage per-group rule |
| V3-8 | GF-4 | §7 aggregate-Constitution head |

Entries: 8 — four sweep grammar repairs, the walls-count repair,
the census-totality rescope, the bundle-rule closed-or-refuse
completion, the aggregate-head semantics confession.

### v4 seed-succession census (round 42-4, 2026-08-06)

One change from v3 (sha256 cd16f4ba...): the Chapter 2 seed body
superseded whole, v2 body (from dfd1ddc1...) -> v3 body (from
870a4d7c..., the seed station's three repairs re-graduated by
ratifying-authority read pass, round record
reviews/rounds/42-4-seed-station-2026-08-06/station-record.md).
The three hunks: the s2.4 presentation-form repair (SF-1/S3-3);
the s2.7 observation-surface repair (GF-1/N2, the gauntlet's
BLOCKING finding); the s2.8 confession completions (SF-3).
Spliced by content anchor; everything outside the seed body
byte-identical to v3.

### v5 bookkeeping census (42-5 minors, 2026-08-06)

Twelve mechanical substitutions from v4 (sha256 a9ac1ed5...),
each citing its 42-5 re-gauntlet finding (report 47934f8c...):
V5-1 = RG-1 (the seed's presentation-form citation re-addressed
from 'the objects section' to 'the consumption ladder', the
kernel site that types the triple; applied identically in the
seed-of-record, station bookkeeping grade); V5-2..8 = RG-2 (the
v3 census's section mispins, s-prefix to section sign); V5-9..12
= RG-4 (four young-prose spaced hyphens to the register's
em-dash). No normative content changed.
