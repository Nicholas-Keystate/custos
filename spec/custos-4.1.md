# Custos 4.1 — Candidate

A standard for governed domains on KERI. Successor edition to
the ratified Custos 4.0 kernel (sha256
9cefdc5d584289ea8391d8069bca26ea38aa82a34f9ae973d80e4d1b7773f315,
anchored at KEL sn 181/182), which it consumes whole and
supersedes at its own effectuation coordinate. Custos 3.3
(sha256
18b0469e731db24f6bca45525828e4417751929306ea21e09977088e08a20ceb)
stands superseded through that lineage. Ratified text is never
edited; this edition was regenerated from a committed input
register, and every departure from its predecessor is accounted
in the appendix of record.
...
by recomputation has a cost. The economics are part of the
design: warranties amortize replay, a warrantor's committed
backing standing in for re-folding until the credible threat of
any stranger folding again is exercised. Judgment stays cheap to
verify precisely because it is expensive to fake. That economy
is the design's claim, stated at the design's grade: its
deployment-scale record — an open replaying population exercising
the credible threat — is a committed, unfinished deliverable of
this standard's record, and no clause below presumes it
discharged.
...
is a defect by this document's own rule. Sections 2 through 16
regenerate the ratified kernel's machinery under that
discipline: scope, reading rules, definitions, the medium, the
objects that cross frames, the finding codomain, standing,
seals, rotation, the governed object classes, the transformation
law, recourse, federation, the openness clause, and succession.
Section 17 specifies the GEL's committed event grammar. The
appendix of record accounts every departure from the predecessor
edition — each delta is ruled, repaired, or confessed, and an
unexplained delta is a defect in this document, not a liberty of
its drafting.

Normative force is carried by keyworded spans under the reading
rules of section 3. The reader who wants the obligations without
the derivations may read Chapter 1, the keyworded spans, and the
appendix of record; the derivations exist so that no obligation
...
restates — from the ratified Custos 4.0 kernel, the edition of
record at sha256 9cefdc5d5842…, whose evaluator sections bind
here by that committed referent: the complete transition system
among the four finding values and its refusal of backward edges,
the canonical ordering and selection of evidence, the distinct
upward currents of defeat and duplicity, first-seen survival,
and the rule that acts consumed as grounds require committed
receipts. Those walls bind with full force; a fold satisfying
the five axioms alone is not yet a conforming evaluator.
Everything a particular domain adds above the ratified walls —
the predicates its law evaluates, the grammar its events speak,
the strata it adopts — is what that domain chose. This standard
specifies the walls and declines to rank the choices: it types
the domain and leaves the taste to the governed.
...
Chapter 1, and no section below re-presents them: a section that
needs the type cites the chapter, and the boundary with KERI —
...
standard's record names general-relative, derived in section 11.
...
## 3. Normative language and reading rules
...
(section 12) is the law of that space.
...
committing a subject to the GARD's covenant set, verified by
whether the successor satisfies the committed clause — are typed
in Chapter 1 §1.2, and the seal ladder of section 9 carries the
...
## 6. The objects that cross frames
...
- **Warranty** — a signed attestation of a computed finding,
  ...
  recomputable.
- **Colored evidence** — evidence traveling with the committed
  inputs by which a frame's judgment colors it; color is not a
  field or intrinsic property of the evidence, and the name is
  participial — evidence that is colored by whichever frame
  folds it, never evidence that owns a color. The object carries
  its color-computation inputs: the colorless base (valid to
...
section 7.5 excludes them by construction rather than by
enumeration.

### 7.2 Pending species and cure
...
the third rung of the ladder of section 7.4, as T1 is the key
...
### 7.4 The duplicity ladder and the two currents
...
section 7: a standing question returns affirmed, defeated with
...
7.5 read constructively: refusal fires where composition is
...
section 14 carries this forward).
...
and the succession acts of section 16 — SHALL anchor in
...
medium — it grounds recourse under section 13.
...
- **on what evidence** — from none to a committed ground with
  ...
  enactment and inherits all of section 13.1: its ground replays
...
recourse machinery of section 13 enacted in key state itself —
...
## 11. The governed object classes
...
two-layer joint of section 10 pointed at a different substrate
...
section 8, which names the schemas its standing law consumes —
...
(section 12) is the medium-grade measure of the passage from
stranger to peer.
...
judged: the knot in section 4's gAID definition — founding law
...
section 10's degenerate policy. Every later event is judged inside
...
the successor exists, the succession rule of section 16 commits
...
illustrations at the grain of section 10's axes: the criterion is
what generalizes; the classes are what this document did with
it.


## 12. The transformation law
...
4. **Confer** (frame-local): what the finding confers in B —
   ...
   whatever B's law says evidence so colored confers.
...
### 13.1 The grounded enactment

Recourse is an act, and acts belong to constructors: a recourse
act is an enactment in the GEL, subject to everything section 8
...
at an uncommitted seam are open surfaces of the section 15
interior.
...
### 13.3 The final rung: recourse against the frame
...
federation duties (section 14). A sovereign cannot be imprisoned, but it can be left —
and in this architecture, leaving takes the proof along.

### 13.4 What this section does not design
...
confession of section 15: who may initiate, how questions are
...
governed protocol, as defined in section 4.
...
## 16. Succession and ratification

The ratified Custos 4.0 kernel is the document of record this
document supersedes, and its bytes are pinned: sha256
9cefdc5d584289ea8391d8069bca26ea38aa82a34f9ae973d80e4d1b7773f315,
...
this document's assembly). Custos 3.3 (sha256
18b0469e731db24f6bca45525828e4417751929306ea21e09977088e08a20ceb)
stands superseded through that lineage. The predecessor remains
...
Section 4 defines the GEL and its anchoring discipline; this
...
own record commits is Exhibit-class evidence for section 15's
confessed frontier: enacted, unrecognized, and honest about the
difference.

**The compact form gate.** A compact, count-code-framed receipt
form — the wire shape by which a warrantor's backing travels
per-receipt — is a committed deliverable gated, in order, on:
...
presentation orders. Until every gate stands, warranted receipts
travel in the heavyweight envelope form, which is complete and
lawful today; the compact form changes cost, never meaning.
...
be derived from the applicable founding-law referent; a grammar
migration not
admitted under the grammar previously in force; compact-form use
while any of its ordered gates remains unstood); refusal-boundary
...
## Appendix of record — drafting-pass accounting (D-3)

Method authority: the consume-and-regenerate ruling. Input
authority: the committed 4.1 input manifest (sha256
b56f842a841e223e57b237994e27783d543ccb14bc4c501573ae21c37c2e49f0),
whose Amendment 1 makes this appendix a two-census register.
Predecessor: the ratified Custos 4.0 kernel (sha256
9cefdc5d584289ea8391d8069bca26ea38aa82a34f9ae973d80e4d1b7773f315).
This appendix is the appendix of record for the melded candidate:
it supersedes the assembly-pass accounting of the defective
concatenation, and no station-level drafting apparatus survives
into it. The census below is generated, not asserted: the
committed generator (the census script in this standard's
operations tree) verifies the delta census, the appendix's
stated totals, and the structure census against the final
candidate bytes under a predeclared rule — a predecessor
nonblank line is a delta if and only if it has no byte-identical
nonblank counterpart anywhere in this candidate — and fails
loudly on any unclaimed line or any stated total that does not
match recomputation. At this candidate's bytes:
of the predecessor's 1,546 nonblank lines, 181 lack a
byte-identical counterpart; each is assigned to exactly one
accounted group below by the generator's predeclared first-match
classifier order — first-match is the exactly-one guarantee —
(scaffolding header 12; embedded abstract 23;
predecessor citations 7; GEL cross-reference and gloss splices
7; color regeneration 9; verb division 2; meld recomposition and
renumbering 118; gauntlet-round repairs 3); zero unexplained.
The census verifier is a ceremony gate: it runs green against
the exact ceremony bytes, invoked with the ceremony digest
pinned, before any anchor is made — a census whose verifier has
not passed at the anchored bytes is stale by definition, and no
edit-history discipline substitutes for that gate. Every other
predecessor sentence was re-derived
and survives verbatim — verbatim survival is re-derivation that
confirmed the predecessor sentence under the rulings in force,
never inertia.

### Delta census

1. Scaffolding header (12 lines) — ruled: never ratified bytes by
   its own declaration; the knot-at-ratification is cured by this
   edition ratifying final-form bytes with no scaffolding.
2. Embedded abstract (23 lines, including the predecessor
   doctrine pair "KERI detects; a GARD appraises" and the
   boundary sentence) — ruled: superseded by this edition's head
   abstract (the adjudicates form, C-2) and Chapter 1's boundary
   restoration; every claim re-derived, none lost. The quoted
   predecessor form appears in this census as a historical
   artifact and nowhere else in this document.
3. Predecessor citations (4 lines plus the succession-section
   line) — ruled: lineage now runs through 4.0 by full digest
   (A-1, A-2).
4. GEL-definition cross-reference and colored-evidence gloss
   splices (3 lines) — ruled: C-6 grammar pointer; C-3 and B-2
   participial gloss at first wire use.
5. Color sites (8 lines) — ruled: C-3 provenance ruling;
   regenerated regime-side; frame-relativity mechanics unchanged.
6. Definitions-section verb site (1 sentence) — ruled: C-2; the
   activity/layer sense of appraisal retained and bounded, the
   doctrine pair regenerated to the office verb with the
   judging-half/acting-half division stated at the site.
7. Meld recompositions (118 lines by the committed generator's
   rule, this integration station) —
   ruled: the manifest's Amendment 1 and the meld instruction;
   the subgroups below describe the recomposition's content (the
   generator assigns some of their lines to the sharper groups
   above, which is why descriptive subgroup arithmetic does not
   sum to the group total; the generator's assignment is the
   census of record):
   - Numbering track (55 lines): predecessor sections 1-15 carry
     as sections 2-16 behind Chapter 1, and every internal
     cross-reference is re-derived under the new numbering,
     including one reference split across a line break.
   - Definitions recomposition (64 lines): the pre-definition
     minimal-case paragraph is retired, superseded by Chapter 1
     §1.1, and replaced by a citing sentence; definitions whose
     subject Chapter 1 types — GARD, the three-logs and
     three-folds typing, the log-and-fold bridge, the finding,
     the seal kinds, the two verbs — are shortened to their
     technical content plus a citation of the typing section;
     every technical clause Chapter 1 lacks (the gAID genesis
     knot with its SHALL spans, the three logs' anchoring
     discipline, the Kever/Tever/Gever naming provenance, frame,
     lens, organ and seat, and the remaining pure technical
     definitions) survives with normative force.
   - Scope recomposition (1 line): the medium-first paragraph's
     opening is recomposed to cite Chapter 1's typing and its
     ownership of the KERI boundary instead of re-presenting
     them.
   - Succession pin repair (3 lines): the concatenation carried
     the Custos 3.3 digest as the pinned bytes of the 4.0
     document of record — a defective splice; repaired to pin
     the 4.0 kernel by its own digest, with 3.3 superseded
     through that lineage (A-1, A-2).
   - Register (2 lines): two relation headers and one vector
     label lowered from full-capital emphasis to ordinary
     emphasis under the register law that reserves capitals for
     BCP 14 keywords.
8. Gauntlet-round repairs (3 lines, the 41-candidate adversarial
   round) — repaired: the definitions section's Constitution
   entry stated identity over identical GELs, contradicting the
   closed replay triple both Chapter 1's second axiom and the
   finding-inputs rule state; regenerated to the triple form,
   with the GEL expressly insufficient alone. The same round
   added the grammar bootstrap and the enlarged vector families
   to the GEL grammar section, and the evidence-grade sentence
   to the abstract's economics paragraph — additions with no
   predecessor counterpart, so they appear in this census only
   through this entry's note.

### Structure census

Per the manifest's Amendment 1, every predecessor section is
accounted as exactly one of regenerated-in-place,
recomposed-into, or retired; a predecessor section surviving
whole into an architecture that makes it redundant is a
first-class defect, and none does.

| 4.0 section | Disposition |
|---|---|
| 1. Scope and non-goals | recomposed-into(section 2): the specifies and does-not-specify lists, the posture commitments, the adversarial-reader paragraph, and the replay-obligation paragraph carry; the embedded abstract and the boundary re-presentation are retired, superseded by the head abstract and Chapter 1 §1.0 |
| 2. Normative language and reading rules | regenerated-in-place(section 3) |
| 3. Definitions | recomposed-into(section 4): the minimal-case paragraph is retired, superseded by Chapter 1 §1.1; typed definitions are shortened against Chapter 1's typing; technical definitions with no Chapter 1 counterpart carry whole |
| 4. The medium | regenerated-in-place(section 5) |
| 5. The objects that cross frames | regenerated-in-place(section 6) |
| 6. The finding codomain | regenerated-in-place(section 7) |
| 7. Standing | regenerated-in-place(section 8) |
| 8. The seal ladder | regenerated-in-place(section 9) |
| 9. Rotation policy | regenerated-in-place(section 10) |
| 10. The governed object classes | regenerated-in-place(section 11) |
| 11. The transformation law | regenerated-in-place(section 12) |
| 12. Recourse | regenerated-in-place(section 13) |
| 13. Federation duties | regenerated-in-place(section 14) |
| 14. The openness clause | regenerated-in-place(section 15) |
| 15. Succession and ratification | regenerated-in-place(section 16), its pin paragraph repaired per delta group 7 |

Additions with no predecessor counterpart: the candidate head and
introduction; Chapter 1 whole (extracted byte-exact from the
graduated seed bytes, sha256
48548304b99e0874c5cf55f8e501a6081129249ad77420b965f03640c4a387a1);
the anchor-grade doctrine in section 9 (E-1); section 17 whole
(C-6, C-7, E-2, E-3); this appendix. Zero predecessor sections
unaccounted; zero additions unaccounted.

Standing obligations this appendix leaves open, named: the
section 17 vector families (equivalence, boundary must-reject,
order) and the composition-manifest fixture (nine entries, per
the sealed final verdict) are committed deliverables, not
discharged claims; semantic color-equivalence remains
fixture-pending; the compact receipt form remains gated. The
52-span finding-codomain signature of section 7 was carried
without semantic delta beyond the ruled verb and color sweeps.
